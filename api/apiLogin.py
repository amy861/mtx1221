...
class ApiLogin:
    def __init__(self):
        logger.info('开始获取登录的url地址：')
        self.url = IP + '/mtx/index.php?s=/index/user/login.html'
        logger.info('登录的url地址：{}'.format(self.url))#这里不能用逗号拼变量
    ...
    def login(self,session,data):
        ...
        '''
        # 发起请求
        logger.info('准备发起登录的请求，请求的参数是{},headers是{}'.format(data,HEADERS))
    ...
    def login_success(self,session):
        ...
        :return:
        '''
        logger.info('准备发起成功的登录请求')
        data = {'accounts': 'yaoyao','pwd':'yaoyao'}
        logger.info('成功的数据信息是{}'.format(data))
        resp_login = session.post(self.url, data=data, headers=HEADERS)
        logger.info('成功请求的响应值是{}'.format(resp_login))
        return resp_login